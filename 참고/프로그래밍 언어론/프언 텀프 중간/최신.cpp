...
map<string, int> variable;	//변수 저장하는 맵
void main()
{
...
int assign(Tokit begin, Tokit end)
{
	...
	{
		Error.push_back(Err(begin[i], "식별자가 위치해야 합니다"));
		return -1;
	}
	...
	{
		Error.push_back(Err(begin[i], "할당 기호가 나오지 않았습니다"));
	}

	//정수와 일치 시도
	if ((begin[i].type == tokenType::integer))
	{
		value = atoi(begin[i].str.c_str());	//문자열을 숫자로 변경해서 저장
		i++;
	}
	else
	{
		Error.push_back(Err(begin[i], "정수가 위치해야 합니다"));
		return -1;
	}

	//모두 정상적으로 끝난 경우엔 할당된 변수와 그 값을 맵에 저장
	variable[var] = value;
...
int _if(Tokit begin, Tokit end)
{
	...
	{
		cout << "닫는 소괄호가 없습니다" << endl;
		exit(0);
	}

	if (m.message == "true")	//if의 논리식이 참일 경우에만 속한 block 실행
	{
		// <block> 이 나오는지 확인
		...
		{
			cout << "블록이 나오지 않았습니다" << endl;
			exit(0);
		}
	}
	else		//if의 논리식이 참이라면 block을 검사하지 않고 토큰만 block 다음으로 넘김
	{
		while (true)
		{
			if (begin[i].type == tokenType::rbracket)	//닫는 중괄호를 만난거면 block이 끝난 것
			{
				break;
			}
			i++;
	...
	if (m.message == "false")	//else의 경우 if의 cmp_expr이 false일 때만 실행함
	{
		/*
...
int block(Tokit begin, Tokit end)
{
	...
	{
		Error.push_back(Err(begin[i], "문장이 나오지 않음"));
		return -1;
	}
...
Message expr(Tokit begin, Tokit end)
{
	...
	if (begin[i].type == tokenType::plus || begin[i].type == tokenType::minus)
	{
		...
		if (oper=="plus")	//덧셈의 경우
		{
			if ((m.type == tokenType::integer) && (temp.type == tokenType::integer))	//정수 + 정수
			{
				m.message = to_string(atoi(m.message.c_str()) + atoi(temp.message.c_str()));
				m.type = tokenType::integer;
			}
			else if ((m.type == tokenType::identifier) && (temp.type == tokenType::integer))	//변수 + 정수
			{
				if (variable.find(m.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
					cout << "선언되지 않은 변수 " << m.message << " 가 사용되었습니다" << endl;
					exit(0);
				}
			...
			else if ((m.type == tokenType::integer) && (temp.type == tokenType::identifier))	//정수 + 변수
			{
				if (variable.find(temp.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
					cout << "선언되지 않은 변수 " << m.message << " 가 사용되었습니다" << endl;
					exit(0);
				}
			...
			else if ((m.type == tokenType::identifier) && (temp.type == tokenType::identifier))	//변수 + 변수
			{
				if (variable.find(m.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
					cout << "선언되지 않은 변수 " << m.message << " 가 사용되었습니다" << endl;
		...
		else //뺄셈일 경우
		{
			...
			else if ((m.type == tokenType::integer) && (temp.type == tokenType::identifier))	//정수 - 변수
			{
				if (variable.find(temp.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
					cout << "선언되지 않은 변수 " << temp.message << " 가 사용되었습니다" << endl;
					exit(0);
				}
...
Message term(Tokit begin, Tokit end)
{
	...
	if (begin[i].type == tokenType::asterisk || begin[i].type == tokenType::divide)
	{
		...
		i += t.useToken;
		if (begin[i-1-t.useToken].type == tokenType::asterisk)	//곱셈일 경우
		{
			if ((m.type == tokenType::integer) && (t.type == tokenType::integer))	//정수 * 정수
			{
				m.message = to_string(atoi(m.message.c_str()) * atoi(t.message.c_str()));
				m.type = tokenType::integer;
			}
			else if ((m.type == tokenType::identifier) && (t.type == tokenType::integer))	//변수 * 정수
			{
			...
			else if ((m.type == tokenType::integer ) && (t.type == tokenType::identifier))	//정수 * 변수
			{
				//변수는 사용 전에 반드시 선언되어야 함
				if (variable.find(t.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
					cout << "선언되지 않은 변수 " << t.message << " 가 사용되었습니다" << endl;
					exit(0);
				}
				m.message = to_string(atoi(m.message.c_str()) * variable[t.message]);
				m.type = tokenType::integer;
			}
			else if ((m.type == tokenType::identifier) && (t.type == tokenType::identifier))	//변수 * 변수
			{
				//변수는 사용 전에 반드시 선언되어야 함
				if (variable.find(m.message) == variable.end())	//변수가 선언되지 않았으면 연산 불가능
				{
...
Message factor(Tokit begin, Tokit end)
{
	...
	{
		cout << "factor) 정수 또는 식별자가 위치해야 합니다" << endl;
		exit(0);
		//return -1;
	}
	m.useToken = i;
...
Message cmp_expr(Tokit begin, Tokit end)
{
	...
	// <expr>과 일치 시도
	m = expr(begin, end);
	//m이 정수였다면 그 값을 바로 mValue에, m이 변수였다면 변수 테이블에서 찾아서 mValue에 저장
	//단, 변수가 선언되지 않고 사용되었으면 불가능
	if ((m.type==tokenType::identifier)&&(variable.find(m.message) == variable.end()))	//변수가 선언되지 않았으면 연산 불가능
	{
		cout << "선언되지 않은 변수 " << m.message << " 가 사용되었습니다" << endl;
		exit(0);
	}
	...
	switch (op.type)
	{
	case tokenType::equal:		// ==연산자의 경우
		result = (mValue == tValue);
		break;
	case tokenType::notequal:	// !=연산자의 경우
		result = (mValue != tValue);
		break;
	case tokenType::greater:	// >연산자의 경우
		result = (mValue > tValue);
		break;
	case tokenType::greaterequal:	// >= 연산자의 경우
		result = (mValue >= tValue);
		break;
	case tokenType::less:		// <연산자의 경우
		result = (mValue < tValue);
		break;
	case tokenType::lessequal:	// <=연산자의 경우
		result = (mValue <= tValue);
		break;
	}
	...
	cout << "CMP_EXPR이 " << result << " 를 감지" << endl;
